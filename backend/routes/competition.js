...
router.post('/rooms', authenticate, async (req, res) => {
  try {
    const { sessionId, timePerQuestion, teacherQuestionCount } = req.body;
    if (!sessionId) {
    ...
    // Count teacher polls with correct_answer
    ...
    // Clamp teacherQuestionCount: 0 = use all, else clamp to available
    const rawTqc = parseInt(teacherQuestionCount);
    const tqc = (!rawTqc || rawTqc <= 0)
      ? 0
      : Math.min(rawTqc, maxTeacherPolls);
    const effectiveTeacherCount = tqc > 0 ? tqc : maxTeacherPolls;

    // Count AI-generated student questions
    const studentQCount = await pool.query(
    ...
    const result = await pool.query(
      `INSERT INTO competition_rooms
         (room_code, session_id, created_by, status, time_per_question, total_questions, teacher_question_count)
       VALUES ($1, $2, $3, 'waiting', $4, $5, $6)
       RETURNING *`,
      [roomCode, sessionId.toUpperCase(), req.user.id, tpq, total, tqc]
    );
...
router.get('/sessions/:sessionId/questions', authenticate, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        teacherPollCount: parseInt(pollsCount.rows[0].count),
...
router.post('/sessions/:sessionId/generate-questions', authenticate, async (req, res) => {
  try {
    ...
    // Retrieve relevant chunks from vector store for this session
    const queryText = 'key concepts, definitions, and important facts from the session material';
    const queryEmbedding = await embeddingService.generateEmbedding(queryText);
    const chunks = await vectorStore.searchSimilar(queryEmbedding, sessionId.toUpperCase(), Math.min(count * 3, 20));
